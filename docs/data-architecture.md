...
| Codex | Codex app-server rate-limit JSON-RPC | `extra` quota rows; dollar/token fields are not applicable; auth/launch failures set `meta.auth_error` and stay visible as an `auth failed` quota row |
