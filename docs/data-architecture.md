...
| Claude Code | OAuth usage endpoint, local usage files, and Claude CLI refresh | `extra.plan_type` from OAuth `subscriptionType` or local `oauthAccount.organizationType`, quota rows, and provider-specific model details; `meta.token_refreshed`, `meta.oauth_retry_status`, and `meta.refresh_error` describe refresh behavior when needed |
| Nous | Portal OAuth account/subscription and token endpoints | `balance` is total usable credits; `spent` is best available period credit drawdown; `extra` exposes subscription, top-up/purchased, monthly, and rollover credit buckets; missing auth sets `meta.skip_reason`, and OAuth refresh sets `meta.token_refreshed`/`meta.oauth_retry_status` when automatic retry is needed |
| Google AI Studio | Cloud Code `loadCodeAssist` entitlement endpoint plus available-models/quota endpoint | `extra.plan_type`/`plan_label`/`plan_source` from `paidTier` or `currentTier`; `extra.models` model quota rows from `fetchAvailableModels`; cached token refreshes before expiry and once after auth/rate-limit endpoint failures when local OAuth client metadata is available |
